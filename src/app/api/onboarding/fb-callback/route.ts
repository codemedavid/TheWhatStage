...
export async function GET(request: NextRequest) {
  ...
  if (!code || !userId) {
    return NextResponse.redirect(
      new URL("/onboarding?fb_error=missing_params", request.url)
    );
  }
  ...
  if (!appId || !appSecret) {
    return NextResponse.redirect(
      new URL("/onboarding?fb_error=not_configured", request.url)
    );
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return NextResponse.redirect(
        new URL("/onboarding?fb_error=token_exchange", request.url)
      );
    }
    ...
    if (!pagesRes.ok) {
      ...
      return NextResponse.redirect(
        new URL("/onboarding?fb_error=pages_fetch", request.url)
      );
    }
    ...
    if (!pages || pages.length === 0) {
      return NextResponse.redirect(
        new URL("/onboarding?fb_error=no_pages", request.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL("/onboarding?fb_connected=true", request.url)
    );
  } catch (err) {
    console.error("FB callback error:", err);
    return NextResponse.redirect(
      new URL("/onboarding?fb_error=unknown", request.url)
    );
  }
}
